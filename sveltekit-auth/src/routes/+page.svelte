...
{:else}
  <div class="p-8 min-w-[32rem] w-min">
    ...
    <p class="my-4">
      ...
      <code class="bg-slate-50 p-1 rounded-md">@edgedb/auth-svelte</code>{" "}
      library.
    </p>

    <p class="my-4">
      To start you need to login with either the custom login page, built using
      api's provided by the auth helper library, or with the EdgeDB's Builtin
      login UI.
    </p>
    ...
    {#if params.error || params.info}
      <div
        class={`${
          params.error ? "bg-rose-100 text-rose-950" : "bg-sky-200 text-sky-950"
